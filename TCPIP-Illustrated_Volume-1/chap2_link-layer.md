...
### 4.数据帧在通过二层交换机、三层交换机和路由器时源MAC和目的MAC地址的变化情况
当数据包经过HUB、**两层交换机**时，数据包的**源MAC不会被更改**，当数据包经过**路由器**时，数据包的**源MAC会被更改**，当数据包经过**三层交换机**的**同一个网段**时数据包的**源MAC不会被更改**，经过**三层交换机的不同网段**时数据包的**源MAC会被更改**。
